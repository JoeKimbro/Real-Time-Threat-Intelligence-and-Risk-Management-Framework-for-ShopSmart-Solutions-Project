...
CREATE TABLE VULNERABILITIES (
  ...
  VULNERABILITY_TYPE VARCHAR(50) CHECK (VULNERABILITY_TYPE IN ('Hardware', 'Software', 'Data', 'People', 'Process')) NOT NULL,
  DESCRIPTION TEXT
);
...
INSERT INTO THREATS (ASSET_ID, THREAT_NAME, THREAT_TYPE, DESCRIPTION, RISK_RATING_ID) VALUES
-- Threats targeting Hardware
(1, 'Unauthorized Access', 'Hardware', 'Hackers attempting to gain unauthorized control over the database server', 1),
(2, 'Denial of Service (DoS) Attack', 'Hardware', 'Overloading employee workstations with excessive traffic causing service disruptions', 2),
(3, 'Malware Infection', 'Hardware', 'Company network switch compromised by malicious software', 3),
-- Threats targeting Software
(4, 'SQL Injection Attack', 'Software', 'Exploiting vulnerabilities in the e-commerce website to execute unauthorized database queries', 4),
(5, 'Cross-Site Scripting (XSS)', 'Software', 'Injecting malicious scripts into the CMS to steal user data', 5),
(6, 'API Abuse', 'Software', 'Excessive API calls causing service degradation and data leakage', 6),
-- Threats targeting Data
(7, 'Data Breach', 'Data', 'Unauthorized access to customer records leading to data exposure', 7),
(8, 'Log Manipulation', 'Data', 'Tampering with transaction logs to cover fraudulent activities', 8),
(9, 'Credential Theft', 'Data', 'Stealing encrypted user credentials through brute-force or phishing attacks', 9),
-- Threats targeting People
(10, 'Social Engineering', 'People', 'Manipulating IT staff into divulging sensitive information', 10),
(11, 'Phishing Attack', 'People', 'Customers tricked into providing login credentials via fraudulent emails', 11),
-- Threats targeting Processes
(12, 'Payment Fraud', 'Process', 'Exploiting weaknesses in payment processing systems to carry out fraudulent transactions', 12),
(13, 'Authentication Bypass', 'Process', 'Exploiting flaws in account authentication mechanisms to gain unauthorized access', 13),
(14, 'Backup Tampering', 'Process', 'Interfering with daily backups to prevent data recovery after an attack', 14);

INSERT INTO VULNERABILITIES (ASSET_ID, VULNERABILITY_NAME, VULNERABILITY_TYPE, DESCRIPTION, RISK_RATING_ID) VALUES
-- Vulnerabilities in Hardware
(1, 'Weak or Default Credentials', 'Hardware', 'Default database credentials make it easier for attackers to gain unauthorized access', 1),
(2, 'Unrestricted Network Traffic', 'Hardware', 'Employee workstations lack network segmentation, making DoS attacks more effective', 2),
(3, 'Unpatched Software', 'Hardware', 'Outdated firmware on networking devices makes them susceptible to malware', 3),
-- Vulnerabilities in Software
(4, 'Unvalidated User Input', 'Software', 'SQL injection vulnerability due to poor input validation on e-commerce website forms', 4),
(5, 'Unescaped User Input', 'Software', 'Lack of input sanitization allows XSS attacks in the CMS', 5),
(6, 'Exposed API Endpoints', 'Software', 'API gateway allows unauthenticated users to query sensitive data', 6),
-- Vulnerabilities in Data
(7, 'Unencrypted Sensitive Data', 'Data', 'Customer records stored in plaintext, increasing exposure risk in case of a breach', 7),
(8, 'Lack of Audit Logging', 'Data', 'Transaction logs can be modified without detection', 8),
(9, 'Weak Password Storage', 'Data', 'User credentials stored with weak hashing algorithms', 9),
-- Vulnerabilities in People
(10, 'Lack of Security Awareness Training', 'People', 'Employees are not trained to recognize social engineering attacks', 10),
(11, 'Email Spoofing Vulnerability', 'People', 'No DMARC policy, allowing phishing emails to appear legitimate', 11),
-- Vulnerabilities in Processes
(12, 'Weak Fraud Detection Systems', 'Process', 'Inadequate fraud prevention tools in payment processing', 12),
(13, 'Broken Authentication Logic', 'Process', 'Incorrect session handling allows authentication bypass', 13),
(14, 'Lack of Backup Integrity Checks', 'Process', 'No verification of backup integrity, increasing risk of tampered backups', 14);
...
(2, 2, 2, 4, 4, 2),  -- Denial of Service (DoS) Attack mapped with Unrestricted Network Traffic (Employee Workstations)
(3, 3, 3, 5, 5, 3),  -- Malware Infection mapped with Unpatched Software (Company Network Switch)
-- Threats targeting Software
